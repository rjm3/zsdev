function addWeapon(%weapon)
{
	...
	$PrevWeapon[%weapon] = $LastWeapon;
	$NextWeapon[%weapon] = $FirstWeapon;
	$PrevWeapon[$FirstWeapon] = %weapon;
	$NextWeapon[$LastWeapon] = %weapon;
...
function isSelectableWeapon(%client,%weapon)
{
	...
	//	if (%weapon != "Knife") {
	//		%clip = Player::GetItemCount(%client,$zs::ammocheck[%weapon]);
	//		if (%ammo == "" || Player::getItemCount(%client,%ammo) > 0 || %clip > 0)
	//			return true;
	//	}
...
$zs::weapondecclip[Beretta] = "Clip9MM";
...
function Weapon::onUse(%player,%item)
{
	if(%player.Station==""){
		...
		if (%ammo == "") {
			...
			Player::mountItem(%player,%item,$WeaponSlot,0);

			//if (Player::getItemCount(%player,%ammo) > 0) 
			//	Player::mountItem(%player,%item,$WeaponSlot);
			//else {
			//	%client = player::getClient(%player);
			//	%clip = Player::getItemCount(%client,$zs::weapondecclip[%item]);
			//	//echo("Clip " @ %clip);
			//	if (%clip) {
			//		//echo("Has Clip");
			//		//player::SetItemCount(%client,%item @ "Ammo",1);
			//		//player::DecItemCount(%client,$zs::weapondecclip[%item],1);
			//		Player::mountItem(%player,%item,$WeaponSlot,0);
			//		
			//	}
			//	else {
...
function TargetingLaser::onUse(%this)
{
	%client = GameBase::getOwnerClient(%this);
	%player = Client::getControlObject(%client);
	echo("Player" @ %player);
	%name = Client::GetName(%client);
	if (!$zs::targeting[%name]) {
		//echo("Not Targeting");
		$zs::targeting[%name] = 1;
		TargetingLaserImage::onFire(%this);
	}
	else
		$zs::targeting[%name] = 0;
		Player::trigger(%player,2,false);
}

function TargetingLaserImage::onFire(%this)
{
	//echo("Target Laser On Fire");
	Player::trigger(%player,2,true);
	//zs::TargetLaser(%this);
}

function zs::TargetLaser(%this) 
{
	//echo("ZS target Laser");
	%client = GameBase::getOwnerClient(%this);
	%name = Client::GetName(%client);
	if ($zs::targeting[%name]) {
		%mounted = Player::GetMountedItem(%client,0);
		//echo("Mounted " @ %mounted);
        	%trans = GameBase::getMuzzleTransform(%this);
		%vel = Item::getVelocity(%this);
		if (%mounted == "Beretta") {
			Projectile::spawnProjectile("greenTarget",%trans,%this,%vel);
			//schedule("zs::TargetLaser("@%this@");",0.1);
		}
		else {
			$zs::targeting[%name] = 0;
		}
...
function InfoPack::onUse(%player,%item)
{
	echo("INFO PACK USED");
}
...
function RepairPack::onUse(%player,%item)
{
	...
	%mounted = Player::GetMountedItem(%player,0);

	zs::getInfo(%player);

	if (Player::getMountedItem(%player,$BackpackSlot) != %item) {
		Player::mountItem(%player,%item,$BackpackSlot);
		player::MountItem(%player,%mounted,0,1);
	}
	else {
...
function RepairKit::onDrop(%player,%item)
{
	// do nothing
}	
...
{
	zs::specialWeapon(%player);
}

function MineAmmo::OnDrop(%player,%item)
{
	// do nothing
}

function Mine::OnDrop(%player,%item)
{
	// do nothing
}
...
function Grenade::onUse(%player,%item)
{
	if($matchStarted) {
		...
		if (!$zs::reloading[%name])
			zs::ReloadWeapon(%client);
	}
}

function Grenade::onDrop(%player,%item)
{
	// do nothing
}	
...
function Beacon::onUse(%player,%item)
{
	...
	%name = Client::GetName(%client);
	if (!$zs::flashlight[%name]) {
		bottomPrint(%client,"<jc> Flashlight <f2> ON");
		player::MountItem(%client,Beacon,2,0);
		$zs::flashlight[%name] = 1;
	}
	else {
		bottomPrint(%client,"<jc> Flashlight <f2> OFF");
		player::unMountItem(%client,2);
		$zs::flashlight[%name] = 0;
	}
}

function Beacon::onDrop(%player,%item)
{
	// do nothing
}	
