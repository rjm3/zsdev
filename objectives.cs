   ...
   if($missionComplete)
   {
		...
		for(%i = 1; %i < getNumTeams() ; %i++) 
      {
		...
		for(%i = -1; %i < getNumTeams(); %i++)
			...
			if ($zombieswon) {
				Team::setObjective(%i, 1, "<jc><F5>UNDEAD won the game!");
				Team::setObjective(%i, 2, "<jc><Bskull_big.bmp>");
			}
			else
				Team::setObjective(%i, 1, "<jc><F5>     HUMANS won the game!");
...
function Game::checkTimeLimit()
{
   ...
   else
   {
	...
	//----------------------------------------------------------------------
		...
		//echo("TIME LEFT " @ floor(%curtimeleft));
		//echo("SIM TIME " @ floor(getSimTime()));
...
function Game::refreshClientScore(%clientId)
{
	...
	Client::setScore(%clientId, "%n\t%t\t" @ zs::GrabTabRank(%clientId) @ "\t" @ zs::GrabKills(%clientId) @ "\t%p %l", %clientId.score + (9 - %team) * 10000);
...
{
   setClientScoreHeading("Player Name\t\x6FTeam\t\xA6Rank\t\xCFKills\t\xEFPing");
...
{
	%this.outArea=1;
	Client::sendMessage(Player::getClient(%this),1,"You left the mission area. You have 5 seconds to return before you are killed!");
	%cl = player::GetClient(%this);
	%name = Client::GetName(%cl);
	$zs::outarea[%name] = 1;
	schedule("zs::checkOutArea("@%cl@");",5);
	//alertPlayer(%this, 3);
}
