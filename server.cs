...
function Server::loadMission(%missionName, %immed)
{
   ...
   echo("Notfifying players of mission change: ", getNumClients(), " in game");
	...
	echo("Setting all players to human team...");
...
function Server::finishMissionLoad()
{
   ...
   schedule("Server::nextMission();", 18000);
	...
	echo("Picking a random zombie... (SERVER LOAD MIS)");
	
	zs::InitMission();

	zs::CheckForZombie();

	echo("ZS INIT LOAD IS " @ $zs::initload);	

	if ($zs::initload == 1) {
		echo(">>>>>>>>> ZS INIT LOAD CONFIRMED");
		schedule("zs::unlockServer();",5);
		$zs::initload = 2;
	}
