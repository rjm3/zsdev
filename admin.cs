...
function Game::menuRequest(%clientId)
{
   ...
   else if($curVoteTopic == "")
   {
	...
	if (Client::GetTeam(%clientId) == 1) {
     		...
     		Client::addMenuItem(%clientId, %curItem++ @ "Zombie", "selzombie");
		if ($zs::infliction >= 1)
			Client::addMenuItem(%clientId, %curItem++ @ "Head Crab", "selheadcrab");
		if ($zs::infliction >= 2)
			Client::addMenuItem(%clientId, %curItem++ @ "Fast Head Crab", "selfheadcrab");
		if ($zs::infliction >= 3)
			Client::addMenuItem(%clientId, %curItem++ @ "Fast Zombie", "selfzombie");
		if ($zs::infliction >= 4)
			Client::addMenuItem(%clientId, %curItem++ @ "Poison Headcrab", "selpheadcrab");
		if ($zs::infliction >= 5)
			Client::addMenuItem(%clientId, %curItem++ @ "Chemical Zombie", "selczombie");
		...
		if (%clientId.isAdmin)
			...
			Client::addMenuItem(%clientId, %curItem++ @ "No Options", "noopt");
	}
	//------------------------------------------------------------------------------------------
   }
...
function remoteSelectClient(%clientId, %selId)
{
   if(%clientId.selClient != %selId)
   {
	...
	remoteEval(%clientId, "setInfoLine", 1, %name);
      	remoteEval(%clientId, "setInfoLine", 2, "Zombies killed: " @ $zs::killedzombies[%name]);
	remoteEval(%clientId, "setInfoLine", 3, "Humans killed: " @ $zs::killedhumans[%name]);
	remoteEval(%clientId, "setInfoLine", 5, "Rank: " @ zs::GrabRankName(%selId));
	remoteEval(%clientId, "setInfoLine", 6, "Score: " @ zs::GrabScore(%selId));
      ...
      //remoteEval(%clientId, "setInfoLine", 6, "     ZOMBIE SURVIVAL (DEV)");
...
function processMenuOptions(%clientId, %option)
{
   ...
   else if(%opt == "observe")
   {
	...
	else if (%opt == "selzombie") {
		zs::changeClass(%clientId,"zombie");
		return;
	}
	else if (%opt == "selheadcrab") {
		zs::changeClass(%clientId,"headcrab");
		return;
	}
	else if (%opt == "selfheadcrab") {
		zs::changeClass(%clientId,"fheadcrab");
		return;
	}
	else if (%opt == "selfzombie") {
		zs::changeClass(%clientId,"fzombie");
		return;
	}
	else if (%opt == "selpheadcrab") {
		zs::changeClass(%clientId,"pheadcrab");
		return;
	}
	else if (%opt == "selczombie") {
		zs::changeClass(%clientId,"czombie");
		return;
	}
