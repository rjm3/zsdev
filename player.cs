...
echo(">>>>>>>>>>>>>> PLAYER.CS SUCCESSFULLY LOADED!");
...
function Player::onDamage(%this,%type,%value,%pos,%vec,%mom,%vertPos,%quadrant,%object)
{
	if (Player::isExposed(%this)) {
		...
		%kteam = Client::GetTeam(%shooterClient);
		%pteam = Client::getTeam(%damagedClient);

		%pname = Client::GetName(%damagedClient);
		$zs::lastpos[%pname] = GameBase::GetPosition(%damagedClient);
		$zs::lastrot[%pname] = GameBase::GetRotation(%damagedClient);

		if (%sname != %pname) {
			$zs::lastmuzzle[%pname] = GameBase::GetMuzzleTransform(%this);
			$zs::lastvel[%pname] = Item::getVelocity(%this);
			$zs::lastplayer[%pname] = %this;
		}

		%sname = Client::GetName(%shooterClient);
                if (%sname != %pname && %kteam != %pteam)
			$zs::lastdamage[%pname] = %sname;
		...
		if (!Player::isDead(%this)) {
			...
			if (%armor == "armorCzombie") {
				echo("IS ARMOR CZOMBIE");
				%trans = GameBase::getMuzzleTransform(%this);
	     			%vel = Item::getVelocity(%player);
				%plusdmg = GameBase::GetDamageLevel(%this) + %value;
				if (%plusdmg > 100) {
					//echo("GREATER THAN 100");
					Projectile::spawnProjectile("CzombieExplode",%trans,%this,%vel);
					GameBase::setDamageLevel(%this,GameBase::getDamageLevel(%this) + 999999);
					Player::BlowUp(%this);
					Player::Kill(%this);
				}
			}
			...
			// POSION HEADCRAB BITE
			if (%type == $PheadcrabDamageType) {
				if (%kteam != %pteam)
					zs::checkPoison(%damagedClient);
			}
			...
			if (%kteam == 0) {
				%curdmg = $zsstats::dmgtoz[%sname];
				$zsstats::dmgtoz[%sname] = %curdmg + %value;
			}
			if (%kteam == 1) {
				%curdmg = $zsstats::dmgtoh[%sname];
				$zsstats::dmgtoh[%sname] = %curdmg + %value;
			}
			...
			if (%type != -1 && %this.shieldStrength) {
  			...
  			if (%value) {
				...
				if (%armor == "armorHuman" || %armor == "armorHumanF")
					%flashval = %value / (%value + 6);
				if (%armor == "armorZombie")
					%flashval = %value / 155;
				else 
					%flashval = %value / 120;
				%flash = Player::getDamageFlash(%this) + %flashval;
